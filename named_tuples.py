...
OrderDetails = namedtuple('OrderDetails',
                          ['order_id', 'customer_first_name', 'customer_last_name', 'employee_first_name',
                           'employee_last_name', 'model_name', 'model_price', 'car_count', 'order_date'])
...
def showModels():
    ...
    for mode in models:
        model = Models(*mode)
        print("|", str(model.model_name).center(25, ' '), "|", str(model.brand_name).center(15, ' '), "|",
              str(model.color_name).center(10, ' '), "|")
        print('-' * 60)
